...
import { Row, Checkbox, Radio, Progress, Col, Tabs, Button, Tooltip } from 'antd'
        ...
        <ModalWrapper style={{ width: '90%', margin: '0 auto', display: 'flex', rowGap: '1rem', flexDirection: 'column' }}>
            <ModalWrapper style={{ backgroundColor: '#494949' }}>
                <div style={{ textAlign: 'center', color: '#f9f9f9' }}>
                    ...
                    <div>
                        {(get(survey, 'isRemain') && get(survey, 'canAttempt')) && <Button type="primary" href={`/surveysTake/${surveyId}?idSubject=${idSubject}&idTimeline=${timelineId}`} style={{ marginTop: 25 }}>{t('take_survey')}</Button>}
